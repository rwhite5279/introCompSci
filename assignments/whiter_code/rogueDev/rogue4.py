...
def createRoom(screen, doorLocation, roomOrientation, stdscr, walkerLoc, placeWalker=False):
    ...
    if roomOrientation in ('left','right','down'):   
        if len(screen[0]) - lowerRightY > 10:    # if there's space at the bottom
            screen[random.randrange(upperLeftX+1, lowerRightX)][lowerRightY] = '+'

    # Place Walker
    if placeWalker:
        walkerLoc = [random.randrange(upperLeftX+1,lowerRightX), \
...
def move(screen, direction, walkerLoc, stdscr):
    """direction will be a, s, d, w"""
    if direction == 'a':
        if screen[walkerLoc[0]-1][walkerLoc[1]] in ['.','+']: walkerLoc[0] -= 1
    elif direction == 's':
        if screen[walkerLoc[0]][walkerLoc[1]+1] in ['.','+']: walkerLoc[1] += 1
    elif direction == 'd':
        if screen[walkerLoc[0]+1][walkerLoc[1]] in ['.','+']: walkerLoc[0] += 1
    elif direction == 'w':
        if screen[walkerLoc[0]][walkerLoc[1]-1] in ['.','+']: walkerLoc[1] -= 1
...
def printScreen(screen, walkerLoc, stdscr):
    for y in range(len(screen[0])):
        for x in range(len(screen)):
            stdscr.addch(y,x,screen[x][y])
    stdscr.addstr(0,len(screen)/2-5,'R O G U E')
    stdscr.addstr(0,50,str(walkerLoc[1])+','+str(walkerLoc[0]))
    stdscr.addch(walkerLoc[1],walkerLoc[0],'@')
...
def main(stdscr):
    ...
    walkerLoc = [16,18]
    walkerLoc = createRoom(theScreen,(15,15),'right',stdscr, walkerLoc, True)
    printScreen(theScreen,walkerLoc, stdscr)
    while True:
        ...
        if chr(ch) == "q": break    # If the character of that key code is the
            ...
            walkerLoc = move(theScreen, chr(ch), walkerLoc, stdscr)
            printScreen(theScreen, walkerLoc, stdscr)
